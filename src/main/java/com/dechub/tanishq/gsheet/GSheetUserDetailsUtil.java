...
public class GSheetUserDetailsUtil {
    ...
    private Sheets sheets; // reuse this client across all calls
    private static final java.util.regex.Pattern TEN_DIGITS = java.util.regex.Pattern.compile("^\\d{10}$");
    private static final long EVENTS_TTL_MS = 10_000; // refresh cache every 10 seconds
    private volatile List<List<Object>> cachedEventsRows;
    private volatile long cachedEventsAt;


    @PostConstruct
    void initOnce() throws Exception {
        this.transport = GoogleNetHttpTransport.newTrustedTransport();
        Credential cred = getCred(this.transport);
        ...
        this.sheets = new Sheets.Builder(transport, JSON_FACTORY, init)
    ...
    private List<List<Object>> getSheetData() throws Exception {
        ValueRange response = execWithBackoff(
                () -> sheets.spreadsheets().values().get(sheetId3, sheetRange).execute(),
                6
        );
...
//    }
    public List<Map<String, Object>> getCompletedEventDetails(String storeCode) throws Exception {
        ...
        ValueRange response = execWithBackoff(
                () -> sheets.spreadsheets().values().get(SheetId11, range).execute(),
                6
        );
        ...
        for (int i = 1; i < values.size(); i++) {
            List<Object> row = values.get(i);
            if (row.size() > 1 && storeCode.equalsIgnoreCase(row.get(1).toString())) {
                Map<String, Object> event = new HashMap<>();
                for (int j = 0; j < headers.size(); j++) {
                    event.put(headers.get(j), j < row.size() ? row.get(j) : "");
                }
    ...
    public List<storeCodeDataDTO> getStoresByRegion(String regionSheetName) throws Exception {
        ...
        String range = regionSheetName + "!A2:A";

        ValueRange response = execWithBackoff(
                () -> sheets.spreadsheets().values().get(sheetId3, range).execute(),
                6
        );
    ...
    public boolean insertSheetData(UserDetailsDTO userDetailsDTO) {
        try {
            ValueRange body = new ValueRange().setValues(this.formInputData(userDetailsDTO));
            ...
            int updated = (resp.getUpdates() != null) ? resp.getUpdates().getUpdatedRows() : 0;

            if (updated > 0) {
                System.out.println("user details excel updated successfully");
                return true;
            } else {
                System.out.println("user details excel update failed");
                return false;
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public boolean insertSheetBrideData(BrideDetailsDTO brideDetailsDTO) {
        try {
            ValueRange body = new ValueRange().setValues(this.formInputBrideData(brideDetailsDTO));

            AppendValuesResponse resp = sheets.spreadsheets().values()
                    .append(sheetId2, "A2", body)
                    .setValueInputOption("RAW")
                    .execute();
            ...
            if (updated > 0) {
                ...
                System.out.println("bride details excel update failed");
                return false;
            }
        } catch (Exception e) {
            e.printStackTrace();
...
//    }
    ...
    public boolean insertSheetInviteesData(InviteesDetailDTO inviteesDetailDTO) {
        try {
            // phone validation
            if (!TEN_DIGITS.matcher(inviteesDetailDTO.getContact()).matches()) {
                System.out.println("⚠️ Invalid contact number: must be 10 digits");
                return false;
            }

            ValueRange body = new ValueRange().setValues(this.formInputInviteesData(inviteesDetailDTO));
            ...
            if (updated > 0) {
                ...
                System.out.println("⚠️ invitees details excel update failed");
                return false;
            }
        } catch (Exception e) {
            e.printStackTrace();
...
//    }
    ...
    private List<List<Object>> addDateTimeAndEventIdToData(
        ...
        if (attendees) {
            ...
            for (int i = 0; i < data.size(); i++) {
                ...
                phone = phone.replaceAll("[\\s-]", "");   // normalize
                if (!TEN_DIGITS.matcher(phone).matches()) {
    ...
    public ResponseDataDTO insertRivaahDetails(RivaahDTO rivaahDTO) {
        ...
        try {
            ...
            ValueRange body = new ValueRange().setValues(this.formInputRivaahData(code, rivaahDTO));

            AppendValuesResponse resp = sheets.spreadsheets().values()
                    .append(sheetId8, "A2", body)
                    .setValueInputOption("RAW")
                    .execute();
            ...
            if (updated > 0) {
                ...
                System.out.println("Rivaah details excel update failed");
                responseDataDTO.setStatus(false);
                responseDataDTO.setMessage("Rivaah details excel update failed");
            }
        } catch (Exception e) {
            e.printStackTrace();
            responseDataDTO.setStatus(false);
    ...
    public boolean insertRivaahUserDetails(String name, String contact) {
        try {
            ValueRange body = new ValueRange().setValues(this.formInputRivaahUserData(name, contact));

            AppendValuesResponse resp = sheets.spreadsheets().values()
                    .append(sheetId9, "A2", body)
                    .setValueInputOption("RAW")
                    .execute();
            ...
            if (updated > 0) {
                ...
                System.out.println("Rivaah user details excel update failed");
                return false;
            }
        } catch (Exception e) {
            e.printStackTrace();
...
//                    System.out.println();
    ...
    public Map<String, String> loadAllStorePasswords() throws Exception {
        ...
        ValueRange response = execWithBackoff(
                () -> sheets.spreadsheets().values().get(sheetId3, "Sheet3!A:D").execute(),
                6
        );
...
//    }
    ...
    private List<List<Object>> getAllEventRows() throws Exception {
        long now = System.currentTimeMillis();
        if (cachedEventsRows != null && (now - cachedEventsAt) < EVENTS_TTL_MS) {
            return cachedEventsRows;
        }
        ValueRange vr = execWithBackoff(
                () -> sheets.spreadsheets().values().get(SheetId11, "Sheet1!A:W").execute(),
                6
        );
        cachedEventsRows = (vr.getValues() == null) ? Collections.emptyList() : vr.getValues();
        cachedEventsAt = now;
        return cachedEventsRows;
    }
    ...
    public List<Map<String, Object>> getEventsForStores(List<String> storeCodes) throws Exception {
        ...
        Set<String> want = storeCodes.stream()
        ...
        for (int i = 1; i < rows.size(); i++) {
            ...
            if (r.size() > 1) {
                String code = String.valueOf(r.get(1)).trim().toUpperCase(); // storeCode at index 1
                if (want.contains(code)) {
                    Map<String, Object> event = new HashMap<>();
                    for (int j = 0; j < headers.size(); j++) {
                        event.put(headers.get(j), j < r.size() ? r.get(j) : "");
                    }
                    Object completed = event.getOrDefault("CompletedEventsDriveLink", "");
                    event.put("completedEvent", completed.toString().trim().length() > 1);
                    out.add(event);
                }
            }
        }
